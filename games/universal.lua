...
local run = function(func) task.wait() func() end
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function()
			return game:HttpGet('https://raw.githubusercontent.com/qwertyui-is-back/CatV5/'..readfile('badvape/profiles/commit.txt')..'/'..select(1, path:gsub('badvape/', '')), true)
		end)
...
-- Global raycast hook that ignores water material (Theme module's decorative water surface)
local IGNORED_RAYCAST_MATERIALS = {
...
oldNamecall = hookmetamethod(game, '__namecall', function(self, ...)
	...
	if method == 'Raycast' and self == workspace then
		...
		for _ = 1, maxIterations do
			local result = oldNamecall(self, currentOrigin, remainingDirection, params)
			if not result then
				return nil
			end

			if not IGNORED_RAYCAST_MATERIALS[result.Material] then
				return result
			end

			-- Hit ignored material, continue ray from hit point
			local traveled = (result.Position - currentOrigin).Magnitude
			local totalDistance = direction.Magnitude
			local remainingDistance = totalDistance - traveled - 0.01
			...
			remainingDirection = direction.Unit * remainingDistance
		end
...
-- Expose for adding/removing ignored materials dynamically
shared.vape.ignoredRaycastMaterials = IGNORED_RAYCAST_MATERIALS
...
getgenv().frictionTable = frictionTable
...
getgenv().updateVelocity = updateVelocity
...
entitylib = loadstring(downloadFile('badvape/libraries/entity.lua'), 'entity')()

weatherlib.Lightning = loadstring(downloadFile('badvape/libraries/Weather/Lightning.lua'), 'Lightning')()
weatherlib.Rain = loadstring(downloadFile('badvape/libraries/Weather/Rain.lua'), 'Lightning')()
weatherlib.Snow = loadstring(downloadFile('badvape/libraries/Weather/Snow.lua'), 'Lightning')()
...
run(function()
	...
	entitylib.targetCheck = function(ent)
		...
		if whitelist.get and not select(2, whitelist:get(ent.Player)) then return false end
...
run(function()
	...
	local function Added(ent)
		...
		EntityArrow.Image = getcustomasset('badvape/assets/new/arrowmodule.png')
...
-- NearBank module - only loads in Bedwars (PlaceId: 6872265039)
if game.PlaceId == 6872265039 then
	run(function()
		...
		local Chests
		local Items = {}
		local bedwars = shared.vape and shared.vape.Bedwars
		...
		local function handleState(Store)
			...
			if not Store then
				...
				for _, v in bedwars.store.inventory.inventory.items do
		...
		NearBank = vape.Categories.Inventory:CreateModule({
			Name = 'NearBank',
			Function = function(callback)
				if callback then
					Chests = collection('personal-chest', NearBank)
					...
					repeat
						...
						local InShop = (bedwars.AppController:isAppOpen('BedwarsItemShopApp') or bedwars.AppController:isAppOpen('TeamUpgradeApp'))
						handleState(not InShop)

						task.wait(0.1)
					until (not NearBank.Enabled)
				else
					table.clear(Items)
				end
			end,
		...
		UIToggle = NearBank:CreateToggle({
			Name = 'UI',
			Function = function(callback)
				if NearBank.Enabled then
					UI.Visible = callback
				end
			end,
			Default = true
...
run(function()
	...
	local function removeOldLightingObject(v)
		if not table.find(newObjects, v) then
			if v:IsA('Sky') or v:IsA('Atmosphere') or v:IsA('BloomEffect') or v:IsA('DepthOfFieldEffect') or v:IsA('ColorCorrectionEffect') or v:IsA('SunRaysEffect') or v:IsA('Clouds') then
				if v.Parent then
					table.insert(oldObjects, v)
					v.Parent = game
				end
	...
	local function applyRealistic()
		...
		lightingService.GeographicLatitude = -7.00

		timeConnection = lightingService:GetPropertyChangedSignal('ClockTime'):Connect(function()
			if Atmosphere.Enabled and lightingService.ClockTime ~= 6.47 then
				lightingService.ClockTime = 6.47
			end
		end)
		...
		local function createCloud()
			local existingCloud = workspace:FindFirstChildOfClass('Terrain'):FindFirstChild('MadeCloud')
			if existingCloud then
				existingCloud:Destroy()
			end
			local cloud = Instance.new('Clouds', workspace:FindFirstChildOfClass('Terrain'))
		...
		task.spawn(function()
			...
			local function transitionWeather(state)
				if not currentCloud then return end
				local tweenInfo = TweenInfo.new(10)
				local cloudGoal = {Density = state.density, Cover = state.cover}
				local cloudTween = tweenService:Create(currentCloud, tweenInfo, cloudGoal)
				cloudTween:Play()
			end
			while Atmosphere.Enabled do
				local waitTime = math.random(15, 20)
				task.wait(waitTime)
				if not Atmosphere.Enabled then break end
		...
		if game.PlaceId ~= 6872265039 then
			pcall(function()
				-- Get all blocks using collection
				...
				storeBlocks, cleanFunc = collection('block')
				...
				if entitylib and entitylib.isAlive and entitylib.character and entitylib.character.RootPart then
					...
					-- Find lowest Y position from blocks
					local attempts = 0
					...
					if waterpos == 0 and #storeBlocks > 0 then
	...
	Atmosphere = vape.Categories.Render:CreateModule({
		...
		Tooltip = 'Applies atmospheric effects to the world'
	})
	Mode = Atmosphere:CreateDropdown({
		Name = 'Mode',
	...
	RemoveClouds = Atmosphere:CreateToggle({
		Name = 'Remove Clouds',
		Function = function()
			if Atmosphere.Enabled then
				Atmosphere:Toggle()
